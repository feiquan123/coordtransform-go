...
// 源函数: BC
func BD09toGCJ02(a Point) Point {
	var b, c = a.Lon - hc, a.Lat - ic
...
// 源函数: T.GK
func gK(a Point, b []float64) Point {
	c := b[0] + b[1]*math.Abs(a.Lon)
...
// 原函数: T.ub
func DB09MctoBD09(a Point) Point {
	var c []float64
	b := Point{math.Abs(a.Lon), math.Abs(a.Lat)}
	for i, v := range jg {
		if b.Lat >= v {
			c = ip[i]
			break
...
// 源函数: T.tb
func BD09toBD09MC(a Point) Point {
	if 180 < a.Lon || -180 > a.Lon || 90 < a.Lat || -90 > a.Lat {
		return Point{0, 0}
	}
	var c []float64
	a.Lon = cE(a.Lon, -180, 180)
	a.Lat = gE(a.Lat, -74, 74)
